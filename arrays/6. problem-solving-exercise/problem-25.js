...
function removeDuplicates(arr) {
  const arr = arr.slice().sort((a, b) => a - b); // Copy and sort the array
  ...
  for (let currentIndex = 1; currentIndex < arr.length; currentIndex++) {
    if (arr[uniqueIndex] !== arr[currentIndex]) {
      ...
      arr[uniqueIndex] = arr[currentIndex]; // নতুন ইউনিক উপাদান সেভ করো।
    }
  }
...
//   if (array[uniqueIndex] !== array[currentIndex]) {
//     uniqueIndex++;
